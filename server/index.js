...
async function scrapeData(url, page) {
  try {
    ...
    $(
      'div.position-relative.ng-tns-c24-0 > main > app-explore > section > div > div.right_sect > app-opportunity-listbox',
    )
      .children('a')
      .each(async function () {
        let comp = {};
        ...
        let prize = $(this)
          .children('.cptn')
          .children('.full-width.ng-star-inserted')
          .children('.tags')
          .children('.inr.ng-star-inserted')
          .text()
          .trim();
    ...
    for (let i = 0; i < compArray.length; i++) {
      ...
      let $ = cheerio.load(html);
      const temp = $(
        'main > app-public-competition > div > div.page_wrapper.with-image > div.section.alt_section.intro.col.p-0.position-relative.scroll-top.tab-infobox > section',
      )
        .children()
        .first()
        .next()
      ...
      if (temp) {
        registerDeadline = $(
          ...
          .text();
        teamSize = $(
          'main > app-public-competition > div > div.page_wrapper.ng-tns-c135-2.with-image > div.section.alt_section.intro.col.p-0.position-relative.scroll-top.tab-infobox.ng-tns-c135-2 > section',
        )
          .children()
          .first()
          .next()
    ...
    $('#future-contests-data')
      ...
      .each(async function () {
        ...
        comp.fees = 'Free';
        (comp.desc = ''), (comp.teamSize = 'Individual');
...
async function getResults() {
  ...
  let data = await scrapeData(
    'https://dare2compete.com/hackathons?filters=open,,all,all&types=oppstatus,teamsize,payment,eligible',
    page,
  );
